...
function! s:_resolve(promise, ...) abort
  ...
  if z#_#promise#is_promise(Result)
...
" Public APIs
function! z#_#promise#new(resolver) abort
...
function! z#_#promise#all(promises) abort
  return z#_#promise#new(function('s:_all', [a:promises]))
endfunction

function! z#_#promise#race(promises) abort
  return z#_#promise#new(function('s:_race', [a:promises]))
endfunction

function! z#_#promise#resolve(...) abort
  let promise = z#_#promise#new(s:NOOP)
  call s:_resolve(promise, a:0 > 0 ? a:1 : v:null)
  return promise
endfunction

function! z#_#promise#reject(...) abort
  let promise = z#_#promise#new(s:NOOP)
  call s:_reject(promise, a:0 > 0 ? a:1 : v:null)
  return promise
endfunction

function! z#_#promise#is_available() abort
  return has('lambda') && has('timers')
endfunction

function! z#_#promise#is_promise(maybe_promise) abort
  return type(a:maybe_promise) == s:DICT_T && has_key(a:maybe_promise, '_vital_promise')
endfunction

function! z#_#promise#wait(promise, ...) abort
...
function! s:_promise_then(...) dict abort
  ...
  let child = z#_#promise#new(s:NOOP)
...
function! s:_on_finally(CB, parent, Result) abort
  ...
  if a:parent._state == s:FULFILLED
    ...
    return z#_#promise#reject(a:Result)
  endif
endfunction
